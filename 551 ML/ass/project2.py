...
class Network:
    ...
    def fit(self, x, y, num_epochs, alpha, optimizer=None, batch_size = 32, val_data = None, val_labels=None):
      ...
      if val_data is None or val_labels is None: #if no validation data is given, use 10% of train data as validation data
        ...
        # val_data, val_labels = x1[0:int(x1.shape[0]/5)], y1[0:int(y1.shape[0]/5)]

      t1 = time.time()
      t_ave = 0
      t_total = 0
      epoch_acc, epoch_error = [], []
      for t in range(num_epochs):
        ...
        if val_data is not None and val_labels is not None:
          ...
          train_acc = accuracy(out2, val_labels)
          epoch_acc.append(train_acc)
          epoch_error.append(error)
          print('train epoch %d/%d   error=%f accuracy=%f' % (t+1, num_epochs, error, accuracy(out2, val_labels)))
        else:
          print('epoch %d/%d   loss=%f ' % (t+1, num_epochs, error))
      
      t_ave = t_total/num_epochs
      print('Finished Training, total time taken: {t_total:.2f}, average time taken per train: {t_ave:.2f}')
      return epoch_acc, epoch_error, train_acc, error, t_total, t_ave
          
def train_val_split(train_data, train_labels, i, fold_size):
  train_data_val = train_data[i*fold_size : (i+1)*fold_size]
  train_labels_val = train_labels[i*fold_size : (i+1)*fold_size]
  train_data_train = np.concatenate([train_data[:i*fold_size], train_data[(i+1)*fold_size:]])
  train_labels_train = np.concatenate([train_labels[:i*fold_size], train_labels[(i+1)*fold_size:]])
  return train_data_val, train_labels_val, train_data_train, train_labels_train

def display_results(i, epoch_acc, epoch_error, val_acc, val_error, acc):
  if i != -1:
    print("Fold %d in the 5-fold Cross-validation:" %(i+1))
  formatted_epoch_acc = [f"{x:.4f}" for x in epoch_acc]
  formatted_epoch_error = [f"{x:.4f}" for x in epoch_error]
  formatted_output = "Train Accuracy: {}, Train Error: {}".format(
      ", ".join(formatted_epoch_acc), ", ".join(formatted_epoch_error))
  print(formatted_output)
  print("Val Accuracy: ", val_acc, "Val Error: ", val_error)
  print("Current Test Accuracy: ", acc)
  plt.plot(epoch_acc)
  plt.xlabel('epoch')
  plt.ylabel('accuracy')

def results(result_current, result_all):
  if result_all.ndim > 1:
    result_current = np.expand_dims(np.array(result_current), axis=0)
    result = np.concatenate((result_all, result_current), axis=0)
  else:
    result_current = np.expand_dims(np.array(result_current), axis=0)
    result = result_current
  return result

epoch_acc_all, epoch_error_all, val_acc_all, val_error_all, acc_all = np.array([]), np.array([]), np.array([]), np.array([]), np.array([])
print(epoch_acc_all.ndim)

net = Network()
net.change_loss(cross_entropy_with_sm, cross_entropy_prime_with_sm)
net.add(FCL(32*32*3, 256))
net.add(ActivationLayer(relu, relu_prime))
net.add(FCL(256, 256))
net.add(ActivationLayer(relu, relu_prime))
net.add(FCL(256, 10))
epoch_acc_1_2, epoch_error_1_2, val_acc_1_2, val_error_1_2, t_total_1_2, t_ave_1_2 = net.fit(train_data, train_labels, num_epochs=150, alpha=0.01, batch_size = 64)
out_1_2 = net.predict(test_data)
test_labels_1_2 = np.argmax(test_labels, axis=1)
acc_1_2 = accuracy(out_1_2, np.array(test_labels_1_2))
display_results(-1, epoch_acc_1_2, epoch_error_1_2, val_acc_1_2, val_error_1_2, acc_1_2)

print("test shapes: ", out_1_2.shape, np.array(test_labels_1_2).shape)
print("test accuracy: ", acc_1_2)
sklearn.metrics.ConfusionMatrixDisplay.from_predictions(test_labels_1_2, out_1_2)
plt.show()